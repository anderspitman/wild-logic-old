from gates import Gate, Nor, Nand, Not
...
class GatedSRLatch(Gate):
    ...
    def get_output(self, name):
        if name == 'Q':
            ...
            raise Exception("invalid Gated SR latch output name")

    def _callback(self):
        pass


class GatedDLatch(Gate):
    def __init__(self, d, enable):
        self._nand0 = Nand(inputs=[d, enable])
        self._nand1 = Nand(inputs=[enable])

        self._not0 = Not([d])
        self._nand1.add_input(self._not0)
    ...
    def get_output(self, name):
        if name == 'Q':
            ...
            raise Exception("invalid Gated D latch output name")
